...
class Config(dict):
    ...
    def __init__(self, preset_config, name="config", read_command_line=True, print_style='table', options={}, helpers={}, show=True, resolve_deps=True):
        ...
        for c in self.preset_config:
            ...
            if type == "dict":
                ...
                self[c] = Config(self.preset_config[c],
                                 name="dict "+c, read_command_line=False, options=option, helpers=helper, show=False, resolve_deps=False)
            else:
                ...
                if key.find(".") >= 0:
                    ...
                    for j in range(num_length-1):
                        if j != num_length - 2:
                            ...
                            if key_list[j] not in subdict:
                                raise AttributeError(
                                    "Cannot set value because key %s is not in the preset_config" % '.'.join(key_list[:j+1]))
                            if key_list[j+1] not in subdict[key_list[j]].preset_config:
                                subdict[key_list[j]].preset_config[key_list[j+1]] = value
    ...
    # 解析配置中的依赖，允许使用 ${path} 作为占位符，引用其他配置项，因此需要解析这类依赖
    def resolve_deps(self):
        def _resolove_deps(config, section, stop=False):
            while not stop:
                stop = True
                re_intp = r'\$\{([\w\.]*)\}'
                if isinstance(section, str):
                    placeholders = re.findall(re_intp, section)
                    for placeholder in placeholders:
                        stop = False
                        nodes = placeholder.split('.')
                        val = config
                        for node in nodes:
                            if node not in val:
                                raise ValueError('Not found placeholder: %s' % placeholder)
                            val = val[node]
                        section = section.replace('${%s}' % placeholder, val)
                elif isinstance(section, dict):
                    for key in section:
                        section[key] = _resolove_deps(config, section[key])
                elif isinstance(section, list):
                    for idx, val in enumerate(section):
                        section[idx] = _resolove_deps(config, val)
            return section

        for key in self.preset_config:
            v = _resolove_deps(self, self[key])
            self.preset_config[key] = v
            self[key] = v
    ...
    def __setattr__(self, name, value, no_check_type=False):
        ...
        if name in RESERVED_NAMES or name in self.preset_config:
            ...
            if type == "dict":
                if name in self.__options:
                    option = self.__options[name]
                else:
                    option = {}
                if name in self.__helpers:
                    helper = self.__helpers[name]
                else:
                    helper = {}
                self[name] = Config(value, name="dict " + name, read_command_line=False, options=option,
                                    helpers=helper, show=False, resolve_deps=False)
            else:
                self[name] = value
            self.preset_config[name] = value
